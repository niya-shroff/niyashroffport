...
// Dynamically import all images from the photography assets folder
const imageModules = import.meta.glob('../assets/photography/**/*.*', { eager: true, query: '?url', import: 'default' });

const staticPhotos: Photo[] = Object.entries(imageModules)
    .filter(([path]) => path.match(/\.(jpg|jpeg|png|webp)$/i))
    .map(([path, url], index) => {
        ...
        return {
            ...
            category: categoryName !== 'photography' ? categoryName.charAt(0).toUpperCase() + categoryName.slice(1) : 'Uncategorized',
            location: categoryName !== 'photography' ? categoryName.charAt(0).toUpperCase() + categoryName.slice(1) : 'Unknown'
        };
    });
...
const Photography = () => {
    ...
    const [dbPhotos] = useState<Photo[]>(staticPhotos);
